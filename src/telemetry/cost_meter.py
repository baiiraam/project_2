...
# Import cost-specific logger
from src.logging_config import get_logger

# Create cost-specific logger
cost_logger = get_logger("cost_meter", log_type="cost")
...
class CostMeter:
    ...
    def __init__(self, db_path: str = "costs.db"):
        ...
        self._init_db()
        logger.info(f"CostMeter initialized with database: {db_path}")
        cost_logger.info(f"COST_METER | Initialized | Database: {db_path}")
    ...
    def record(
        ...
        # Log to both main logger and cost-specific logger
        logger.debug(f"Recorded cost: {provider}/{model} - ${cost_usd:.6f} ({prompt_tokens}+{completion_tokens} tokens)")
    ...
    def clear(self) -> None:
        ...
        with sqlite3.connect(self.db_path) as conn:
            conn.execute("DELETE FROM cost_records")
        logger.info("Cost records cleared")
        cost_logger.info("COST_METER | All records cleared")
