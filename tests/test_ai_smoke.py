...
def test_identify_ingredients_returns_empty_when_meal_not_recognized(
    fake_vlm, sample_image
):
...
def test_compute_totals_multiple_ingredients(fake_nutrition):
    ...
    facts = {
        n: fake_nutrition.lookup(n)
        for n in ["white rice (cooked)", "grilled chicken breast", "broccoli"]
    }
    n = compute_totals(ings, facts)
